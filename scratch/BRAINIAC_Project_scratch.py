...
class BrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            print(f"Error loading {self.scan_paths[idx]}: {e}")
            raise
...
def train_and_evaluate(scan_paths, labels, epochs=5, batch_size=2):
    ...
    # Save model
    torch.save(model.state_dict(), os.path.join(preprocessed_dir, 'model_trained.pth'))
    ...
    print(cr)
    print("\n\U0001F50D Sample prediction probabilities:")
    print(all_probs[:10])
    ...
    plt.title('ROC Curve')
    plt.legend()
    plt.savefig(os.path.join(preprocessed_dir, 'roc_curve.png'))
    plt.show()
    ...
    plt.ylabel('Count')
    plt.savefig(os.path.join(preprocessed_dir, 'probability_distribution.png'))
    plt.show()
